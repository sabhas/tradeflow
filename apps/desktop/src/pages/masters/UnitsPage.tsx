...
export function UnitsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['units'],
    enabled: canRead,
    queryFn: () => apiFetchData<Row[]>('/units'),
  });
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (editing) {
        ...
        await apiFetch('/units', { method: 'POST', body: JSON.stringify({ code, name }) });
      }
    },
    onSuccess: () => {
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        <h1 className="text-2xl font-semibold text-slate-800 dark:text-slate-100">Units of measure</h1>
        {canWrite && (
          <button
            type="button"
                ...
                {canWrite && (
                  <td className="px-4 py-3 text-right">
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (window.confirm('Delete this unit?')) del.mutate(r.id);
                      }}
                    >
                      Delete
                    </button>
