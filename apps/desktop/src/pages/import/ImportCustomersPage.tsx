...
export function ImportCustomersPage() {
  ...
  const mutation = useMutation({
    mutationFn: async (f: File) => {
      ...
      fd.append('file', f);
      const res = await fetch(`${getApiBase()}/import/customers`, {
        method: 'POST',
  ...
  if (!canWrite) {
    return <p className="text-slate-600">You need customers write permission to import.</p>;
  }
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-semibold text-slate-800 dark:text-slate-100">Import customers</h1>
      <p className="mt-1 text-slate-600 dark:text-slate-400">
        Columns: name, type (retailer | wholesaler | walk_in), optional contact fields, creditLimit, paymentTerms and
        taxProfile (names must match master data).
      </p>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <button
          ...
          onClick={() => downloadAuthenticatedFile('/import/customers/template?format=xlsx', 'customers-template.xlsx')}
        >
          Template (.xlsx)
        </button>
        <button
          type="button"
          className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
          onClick={() => downloadAuthenticatedFile('/import/customers/template?format=csv', 'customers-template.csv')}
        >
          Template (.csv)
        </button>
      </div>
