...
type ProofOfDeliveryInput = z.infer<typeof proofOfDeliverySchema>;
...
export async function createDeliveryNote(req: Request, body: CreateDeliveryNoteInput): Promise<ControllerResult> {
  const b = body;
  try {
...
export async function addProofOfDelivery(req: Request, body: ProofOfDeliveryInput): Promise<ControllerResult> {
  if (!canWritePod(req)) {
