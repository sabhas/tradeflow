...
import { postPurchaseReturnJournal } from '../services/accountingPosting';
...
export async function createPurchaseReturn(req: Request, body: CreatePurchaseReturnInput): Promise<ControllerResult> {
  const b = body;
...
export async function updatePurchaseReturn(req: Request, body: UpdatePurchaseReturnInput): Promise<ControllerResult> {
  try {
    const row = await runInTransaction(async (manager) => {
...
export async function postPurchaseReturn(req: Request): Promise<ControllerResult> {
  try {
    await runInTransaction(async (manager) => {
      ...
      const totals = await computePurchaseDocumentTotals(manager, pr.supplierId, linesForCalc, pr.discountAmount);
