...
import {
  ...
  mapProductRow,
} from '../../../shared/utils/importColumnMap';
import type { SheetTable } from '../../../shared/utils/tabularFile';
import {
  applyMovement,
...
async function findCustomerTypeNameByName(em: EntityManager, name: string): Promise<string | null> {
  const n = name.trim();
  if (!n) return null;
  const row = await em
    ...
    try {
      await runInTransaction(async (em) => {
        ...
        const payload = {
          ...
          manufacturerCode: row.manufacturerCode?.trim() || null,
          shortName: row.shortName?.trim() || null,
          genericName: row.genericName?.trim() || null,
          packing: row.packing?.trim() || null,
          hsCode: row.hsCode?.trim() || null,
          retailPrice: row.retailPrice ?? '0',
          isActive: row.isActive ?? true,
          isHerbal: row.isHerbal ?? false,
          isNarcotic: row.isNarcotic ?? false,
          isFridged: row.isFridged ?? false,
          isSurgical: row.isSurgical ?? false,
          saleType: row.saleType?.trim() || null,
          sroSchedule: row.sroSchedule?.trim() || null,
          sroItemSerial: row.sroItemSerial?.trim() || null,
        ...
        const p = repo.create({
          ...
          manufacturerCode: valid.data.manufacturerCode ?? undefined,
          shortName: valid.data.shortName ?? undefined,
          genericName: valid.data.genericName ?? undefined,
          packing: valid.data.packing ?? undefined,
          hsCode: valid.data.hsCode ?? undefined,
          retailPrice: valid.data.retailPrice ?? '0',
          isActive: valid.data.isActive ?? true,
          isHerbal: valid.data.isHerbal ?? false,
          isNarcotic: valid.data.isNarcotic ?? false,
          isFridged: valid.data.isFridged ?? false,
          isSurgical: valid.data.isSurgical ?? false,
          saleType: valid.data.saleType ?? undefined,
          sroSchedule: valid.data.sroSchedule ?? undefined,
          sroItemSerial: valid.data.sroItemSerial ?? undefined,
        });
        await repo.save(p);
      });
      successCount++;
...
export async function importCustomersFromSheets(
  ...
  for (const raw of table.rows) {
    ...
    try {
      await runInTransaction(async (em) => {
        const paymentTermsId = await findPaymentTermsByName(
          em,
...
> {
  ...
  if (invSheet?.rows.length) {
    ...
    const groups = new Map<
      GroupKey,
      { warehouseCode: string; movementDate: string; lines: z.infer<typeof openingInventoryRowSchema>[] }
    >();
    let r = 2;
  ...
  if (journalSheet?.rows.length) {
    if (!canJournal) {
      ...
      for (const [, lines] of jgroups) {
        try {
          const normalized = lines.map((l) => normalizeJournalLine({ debit: l.debit, credit: l.credit }));
