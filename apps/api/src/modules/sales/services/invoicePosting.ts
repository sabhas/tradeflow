...
import { decimalAdd, decimalSub, parseDecimalStrict } from '../../../shared/utils/decimal';
import { toIsoDateString } from '../../../shared/utils/date';
import { moneyCmp, moneySub } from '../../../shared/utils/money';
import { getCustomerCreditExposure, getInvoiceAmountAllocated } from './salesCustomerBalance';
import {
  postSalesCreditNoteJournal,
  postSalesInvoiceJournal,
} from '../../accounting/services/accountingPosting';
import { addDaysIso } from './salesTotals';
import { assertDateNotPeriodLocked } from '../../accounting/services/periodLock';
import { assertCreditNoteLinesMatchOriginal } from '../../inventory/services/productBatchControls';
import {
...
async function assertCreditNoteReadyToPost(manager: EntityManager, inv: Invoice): Promise<void> {
  ...
  if (orig.warehouseId !== inv.warehouseId)
    throw new Error('Warehouse must match original invoice (return to same site)');
