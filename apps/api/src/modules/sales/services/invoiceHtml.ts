...
import { roundAmountString } from '../../../shared/utils/rounding';
...
export function renderInvoiceBody(opts: InvoicePrintData): string {
  ...
  const notesBlock =
    cfg.showNotes && inv.notes ? `<p style="margin-top:12px;font-size:13px">${esc(inv.notes)}</p>` : '';

  const rows =
    lines
      .map((l) => {
        const name = productNames.get(l.productId) ?? l.productId;
        const batch = l.batchCode?.trim() || '';
  ...
  ${termsBlock}
  <table>
    <thead><tr><th>Product</th><th>Batch</th><th>Qty</th><th>Bonus</th><th>Price</th><th>Disc</th><th>Tax</th><th>Line</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>
  <div class="totals">
