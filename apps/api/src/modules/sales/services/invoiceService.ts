...
import { CompanySettings, Customer, Invoice, InvoiceLine, InvoiceTemplate } from '@tradeflow/db';
...
import { syncCreditNoteLinesWithOriginal, buildOriginalLineMap } from './invoiceCreditNoteLines';
import {
  assertCreditNoteLinesMatchOriginal,
  enforceProductBatchControls,
} from '../../inventory/services/productBatchControls';
import { getCompanySettingsRow } from '../../settings/services/companySettings';
import { resolveInvoiceLineUnitPrices, type InvoiceLinePricingResolved } from './invoicePricingService';
import { calculateBonus } from './bonusService';
import {
  assertNoOtherInvoiceForSalesOrder,
  assertSalesOrderConfirmedForInvoice,
} from './salesOrderInvoiceGuard';
import { buildInvoicesBatchPrintHtml, renderInvoiceBody, type InvoicePrintData } from './invoiceHtml';

type CreateInvoiceInput = z.infer<typeof createInvoiceSchema>;
type UpdateInvoiceInput = z.infer<typeof updateInvoiceSchema>;
type PrintInvoicesBatchInput = z.infer<typeof printInvoicesBatchSchema>;

function lineBatchExpiryFields(line: { batchCode?: string | null; expiryDate?: string | null }): {
  batchCode?: string;
  expiryDate?: string;
} {
...
export async function loadCompanyForPrint(): Promise<CompanySettings> {
  const company = (
...
export async function printInvoicesBatch(body: PrintInvoicesBatchInput): Promise<string> {
  let ids: string[] = [];
  if (body.mode === 'ids') {
    ids = body.ids;
  } else {
  ...
  return buildInvoicesBatchPrintHtml(parts);
}
