...
import { batchExpiryKey } from './productBatchControls';
...
export function buildOriginalLineMap(lines: InvoiceLine[]): Map<string, { productId: string; batchCode?: string; expiryDate?: string }> {
  return new Map(
    lines.map((l) => [
      l.id,
      {
