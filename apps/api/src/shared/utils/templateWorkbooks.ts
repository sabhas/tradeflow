...
export async function customerImportTemplateBuffer(): Promise<Buffer> {
  ...
  ws.addRow(['Acme Retail', 'retailer', '+123', 'a@example.com', '1 Main St', '1000', '', '']);
  const buf = await wb.xlsx.writeBuffer();
  return Buffer.from(buf);
}
...
export function openingInventoryTemplateCsv(): string {
  return ['warehouseCode,movementDate,productSku,quantity,unitCost', 'MAIN,2025-01-01,SKU-001,100,0.50'].join(
    '\n'
  );
}
